...
float TrajectoryReaderH5::getTime() const
{
    vesDEBUG(__PRETTY_FUNCTION__)
...
std::size_t TrajectoryReaderH5::numParticles() const
{
    vesDEBUG(__PRETTY_FUNCTION__)
...
bool TrajectoryReaderH5::isAnisotropic() const
{
    vesDEBUG(__PRETTY_FUNCTION__)
    if(frames.empty()) throw std::logic_error("no frame was read");
...
std::map<std::string,std::string> TrajectoryReaderH5::particleLineTokens(std::string line) const
{
    vesDEBUG(__PRETTY_FUNCTION__)
