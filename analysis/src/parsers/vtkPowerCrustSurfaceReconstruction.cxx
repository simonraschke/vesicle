/*=========================================================================
  ...
  Module:    $RCSfile: vtkPowerCrustSurfaceReconstruction.cxx,v $
  Language:  C++
  Date:      $Date: 2002/07/11 15:05:51 $
...
void pcInit ();		// EPRO added
void free_heap ();	// EPRO added
...
long *shufmat;
static long mat_size = 0;	// EPRO set to global to reinitialize

void make_shuffle(void){
    long i,t,j;
 //   static long mat_size = 0; // EPRO commented
...
static neighbor p_neigh={0,0,0};	// EPRO added set to global
...
//#define two_to(x) ( ((x)<20) ? 1<<(x) : ldexp(1,(x)) )	// EPRO commented
...
int reduce_inner(basis_s *v, simplex *s, int k) {
    ...
    basis_s *snibv;
    neighbor *sni;
//    static int failcount;		// EPRO commented - next line added
    static int failcount = 0;	// EPRO added
...
void trans(point z, point p, point q) {
    ...
    for (i=0;i<pdim;i++)
    {
        // if( sizeof(z)/sizeof(z[0]) >= i+rdim)
        {
        //   std::cout << "size " << sizeof(z)/sizeof(z[0]) << "   i " << i << "   rdim " << rdim << std::endl;
...
//    static neighbor p_neigh={0,0,0}; // EPRO commented - set to global to reinitialize
...
//    exact_bits = (int)(DBL_MANT_DIG*log(FLT_RADIX)/log(2)); // cast to int added by TJH // EPRO commented
...
void init_heap(int num)
{
    ...
    //heap_A = (struct heap_array *)calloc(num , sizeof(struct heap_array));	// EPRO commented
    heap_A = (struct heap_array *)calloc(num + 1, sizeof(struct heap_array));	// EPRO added
    heap_size = 0;
    heap_length = num;
    // TJH: added this if statement
    if(DFILE)
